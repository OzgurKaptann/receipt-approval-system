...
class Deposit(Base):
    ...
    document_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('uploaded_documents.id'), unique=True, nullable=False)

    mt_account_id: Mapped[str] = mapped_column(String(80), nullable=False)

    src_amount: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False)
    ...
    dst_amount: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False)
